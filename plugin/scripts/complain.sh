...
if [[ -n "$STATE_FILE" && -f "$STATE_FILE" ]]; then
  if command -v jq >/dev/null 2>&1; then
    HAS_STATE=1
    # Single jq call: emit all 3 fields as TSV, empty string where unset.
    IFS=$'\t' read -r S_EN S_FQ S_VO < <(jq -r '
      [
        (if has("enabled")   then (.enabled|tostring)  else "" end),
        (if has("frequency") then .frequency           else "" end),
        (if has("voice")     then .voice               else "" end)
      ] | @tsv
    ' "$STATE_FILE" 2>/dev/null) || { S_EN=""; S_FQ=""; S_VO=""; }
    [[ -n "${S_EN:-}" ]] && ENABLED="$S_EN"
...
# Single jq call: (optional merge) -> pick by $RANDOM index.
